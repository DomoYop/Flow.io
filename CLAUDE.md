...
- **La DRAM interne est la ressource rare de cette carte** — pas la flash (47 %), pas la PSRAM (7,9 Mo libres). **226 Ko en tout**, dont ~114 Ko de statique : il reste une vingtaine de kilo-octets de tas interne, et le plancher observé descend à 8 Ko. Deux pannes le 20/08/2026 pour l'avoir ignoré : le serveur d'assets refuse de servir sous **10 240 o libres ou 6 144 o de plus gros bloc** (`shouldRejectAssetByFreeHeap_`) et répond `{"ok":false,"err":{"code":"Busy"}}` avec l'en-tête `X-Flow-Busy-Reason: low_memory`. **Mesurer avant d'allouer** : `GET /api/system/heap` sépare interne et PSRAM. Ne pas se fier aux seuils de `SystemMonitorModule`, qui lisent `MALLOC_CAP_8BIT` — donc les deux tas agrégés — et ne peuvent jamais se déclencher. Détail : [audit-paniques-flash-cache.md](docs/notes/audit-paniques-flash-cache.md).
- **Table de partitions 16 Mo** ([partitions_flowios3_ota_16mb.csv](partitions_flowios3_ota_16mb.csv), depuis le 20/08/2026) : `app0`/`app1` 4 Mo, **`spiffs` 2 Mo** (le contenu web pèse 317 Ko une fois gzippé par `prepare_spiffs_data.py`), `spiffs_b` 2 Mo réservée à une future écriture A/B non destructive, `coredump` **256 Ko**. `spiffs_b` doit rester **après** `spiffs` dans la table : `SPIFFS.begin()` comme `Update.begin(U_SPIFFS)` prennent la première partition de ce sous-type, sans regarder le label. Un OTA ne met jamais la table à jour — la changer exige un flash USB, et une carte encore en 8 Mo refuse l'image de 2 Mo (contrôle `isize != partitionSize`, comportement voulu).
...
- [audit-paniques-flash-cache.md](docs/notes/audit-paniques-flash-cache.md) — **les paniques d'août 2026 étaient des débordements de pile** (`exc_cause = 65`, exception de débogage levée par `CONFIG_FREERTOS_WATCHPOINT_END_OF_STACK`), pas un problème de cache flash ni de SPIFFS. **Corrigé et vérifié sur cible en 4.4.3** : piles `mqtt` 6656, `sysmon` 4096, `wifiprov` 4096, `eventbus` 3584, financées en réduisant `ethernet` 6144→3072 et `hmi` 6144→4096, qui consommaient 800 et 1 764 octets. Trois pièges à connaître avant de toucher au dimensionnement : un **`#if` de profil masque la valeur active** (modifier `kWaveshareESP32S3MqttCapacity` sans toucher `MQTTModule.h` n'a aucun effet), **une grosse pile coûte deux fois** (la mémoire prise et le plus gros bloc détruit — le serveur d'assets refuse de servir sous 6 144 o contigus), et les **cinq piles en PSRAM sont délibérées**, pas un oubli. Contient aussi le relevé complet des marges par tâche et la méthode de mesure.
- [ota-spiffs-gzip-bilan.md](docs/notes/ota-spiffs-gzip-bilan.md) — **point d'entrée du chantier OTA SPIFFS compressé** : image gzip ramenée en mémoire puis connexion fermée, vérification taille + CRC-32 avant la première écriture, essai à blanc (`dry=1`). L'OTA est **fiable depuis le 20/08/2026** — les corruptions répétées de la nuit du 18/08 étaient la conséquence des débordements de pile ci-dessus, pas une cause propre au SPIFFS ; l'avertissement « ne pas écrire en flash » qui figurait ici est levé. Outillage à conserver : `src/Core/CoreDumpInfo.*` (résumé JSON du vidage avec registres a0-a15/EPCx via `GET /api/system/coredump` ; la route `/raw` tronque toujours, cause non cherchée), `src/Core/SpiffsAccessLock.*`, et la console de secours **`/rescue`** embarquée dans le firmware, indépendante de SPIFFS. Historique détaillé dans [ota-spiffs-reduction-volume.md](docs/notes/ota-spiffs-reduction-volume.md).
...
- [pression-encrassement-filtre-et-securites.md](docs/notes/pression-encrassement-filtre-et-securites.md) — `pressure_low` ne coupe plus la filtration : le seuil devient la **pression de service filtre propre** (apprise seule, modifiable à la main) et sert de référence à une alerte d'encrassement relative (**implémenté** le 23/08/2026, migration NVS v3→v4 automatique). L'alarme d'origine venait de PoolMaster, où le manomètre remplaçait un flowswitch absent ; ce rôle revient au **flowswitch**, qui coupe désormais la pompe **et le chauffage** sur manque de débit — ce dernier n'était couvert que par ricochet. Seules deux causes arrêtent encore la pompe : surpression mécanique et manque de débit, toutes deux latchées **volontairement** auto-verrouillantes, avec verrouillage franc après trois re-trips. Piège à connaître : le trip restant à 1,80 bar, une référence apprise au-dessus de ~1,3 bar fait plafonner l'alerte de lavage (`LOGW` « Fouling threshold clamped ») — relever `pressure_high_th` devient alors nécessaire.
