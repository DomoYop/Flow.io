...
- [ota-spiffs-reduction-volume.md](docs/notes/ota-spiffs-reduction-volume.md) — transfert OTA de l'image SPIFFS compressée (25× moins : 329 Ko contre 8,3 Mo). Générateur, négociation `.gz` et décodeur `tinfl` en place mais **désactivés** (`FLOW_OTA_SPIFFS_GZIP = 0`) : trois essais sur cible, trois blocages, cause non identifiée. Contient les hypothèses testées — dont une **infirmée par test** — et les trois mesures à faire pour reprendre.
